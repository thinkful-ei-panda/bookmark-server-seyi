...
describe('Bookmarks Endpoints', () => {
  ...
  describe('DELETE /bookmarks/:id', () => {
    it('removes the bookmark by ID from the store', () => {
      const secondBookmark = store[1]
      const expectedBookmarks = store.filter(s => s.id !== secondBookmark.id)
      return supertest(app)
        .delete(`/bookmarks/${secondBookmark.id}`)
        .set('Authorization', `Bearer ${process.env.API_TOKEN}`)
        .expect(204)
        .then(() => {
          expect(store).to.eql(expectedBookmarks)
        })
    })
  ...
  describe('POST /bookmarks', () => {
    ...
    it('adds a new bookmark to the store', () => {
      ...
      return supertest(app)
        ...
        .then(res => {
          expect(store[store.length - 1]).to.eql(res.body)
        })
    })
  })
})